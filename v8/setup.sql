...
CREATE OR REPLACE FUNCTION get_related_sketches(A_SKETCH_ID bigint) RETURNS SETOF similar_sketch AS
$func$
  SELECT
  s.sketch_id,
  COUNT(*)::float / NULLIF(MAX(s.num_geometries) + MAX(a.num_geometries) - COUNT(*), 0) AS similarity
  FROM (SELECT * FROM sketch_geometry_hashes WHERE sketch_id != A_SKETCH_ID) s
  INNER JOIN (SELECT * FROM sketch_geometry_hashes WHERE sketch_id = A_SKETCH_ID) a ON s.poly_hash = a.poly_hash
...
$func$
  SELECT
    a.sketch_id,
    HASH_WEIGHT * MAX(COALESCE(x.similarity, 0))
  ...
  LEFT JOIN get_related_sketches(A_SKETCH_ID) x ON a.sketch_id = x.sketch_id
